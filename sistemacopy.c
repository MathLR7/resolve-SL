...
int menu(float *matrixA, float *matrixAcopy, float *vectorB, float *vectorBcopy, int matrixLenght){         //apresenta o menu
    ...
    while(!(option>0 && option<5)){
        ...
        switch (option){
            ...
            default:
                printf("\nisso eh numero valido por acaso????OTARIO, QUER FERRAR MEU CODIGO NE SEU PALERMA????\nAgora vou te dar uma lista de motivos do pq nao devia ter feito isso:\nI-eh coisa de otario\nII-tinha de ser flamenguista neh?\nIII-impostoehrobo\nIV-oRestodaListataem:encurtador.com.br/cdmBT\n");
